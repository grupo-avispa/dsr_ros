...
void PersonAgent::person_callback(const person_msgs::msg::PersonArray::SharedPtr msg)
{
  ...
  for (auto detection : msg->people) {
    ...
    auto it = std::find_if(
      person_nodes.begin(), person_nodes.end(),
    ...
    if (it == person_nodes.end() ) {
      RCLCPP_INFO(
        this->get_logger(), "Person detected [%s] with id [%s]",
        person_name.c_str(), person_id.c_str());
      // TODO(opfernandez): ¿Queremos que pasen personas sin identificar al DSR?
      if (!person_name.empty()) {
        ...
        G_->add_or_modify_attrib_local<pose_x_att>(
          person_node, static_cast<float>(center_point.pose.position.x));
        G_->add_or_modify_attrib_local<pose_y_att>(
          person_node, static_cast<float>(center_point.pose.position.y));
        G_->add_or_modify_attrib_local<pose_angle_att>(
          person_node, static_cast<float>(tf2::getYaw(center_point.pose.orientation)));
        G_->add_or_modify_attrib_local<timestamp_att>(
          person_node, static_cast<int>(msg->header.stamp.sec));
        G_->add_or_modify_attrib_local<initstamp_att>(
          person_node, static_cast<int>(msg->header.stamp.sec));
        G_->add_or_modify_attrib_local<posture_att>(person_node, detection.posture);
        G_->add_or_modify_attrib_local<source_att>(
          person_node, static_cast<std::string>("robot"));
        if (auto id = G_->insert_node(person_node); id.has_value()) {
          ...
          // TODO(opfernandez): Remove with_edge from the codes
          auto edge_with = DSR::Edge::create<is_with_edge_type>(
            person_node.id(), robot_node.value().id());
      ...
      if (!person_name.empty()) {
        G_->add_or_modify_attrib_local<identifier_att>(
          *it, static_cast<std::string>(person_name));
      }
      G_->add_or_modify_attrib_local<pose_x_att>(
        *it, static_cast<float>(center_point.pose.position.x));
      G_->add_or_modify_attrib_local<pose_y_att>(
        *it, static_cast<float>(center_point.pose.position.y));
      G_->add_or_modify_attrib_local<pose_angle_att>(
        *it, static_cast<float>(center_point.pose.position.z));
      G_->add_or_modify_attrib_local<timestamp_att>(
        *it, static_cast<int>(msg->header.stamp.sec));
      G_->add_or_modify_attrib_local<posture_att>(
        *it, static_cast<std::string>(detection.posture));
...
void PersonAgent::remove_callback()
{
  ...
  for (const auto & person : person_nodes) {
    ...
    if (interact_edge.has_value()) {
      RCLCPP_WARN(
        this->get_logger(),
        "The person [%s] is interacting, so it will not be removed", person.name().c_str());
      continue;
    }
    ...
    if (timestamp.has_value() && (now.seconds() - timestamp.value()) >= timeout_) {
      ...
      RCLCPP_INFO(
        this->get_logger(),
        "The person [%s] has been removed from the DSR graph", person.name().c_str());
    }
  }
