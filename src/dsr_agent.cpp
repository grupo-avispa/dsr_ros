...
dsrAgent::dsrAgent(): Node("dsr_agent"){
	...
	for (auto type : data[ros_topic_]){
		generic_sub_ = create_generic_subscription(
						...
						std::bind(&MinimalPublisher::serial_callback, this, std::placeholders::_1)
						);
	}
}
...
void dsrAgent::get_params(){
	...
	RCLCPP_INFO(this->get_logger(), "The parameter ros_topic is set to: [%s]", ros_topic.c_str());
...
void dsrAgent::create_and_insert_node(const std::string &name){
	RCLCPP_ERROR(this->get_logger(), "%s node not found", name);
...
void dsrAgent::modify_battery_attibutes_and_update(const std::optional<DSR::Node> &node, 
	...
	G_->add_or_modify_attrib_local<battery_power_supply_status_att>(node.value(), msg.power_supply_status);
	G_->add_or_modify_attrib_local<battery_power_supply_health_att>(node.value(), msg.power_supply_health);
	G_->add_or_modify_attrib_local<battery_power_supply_technology_att>(node.value(), msg.power_supply_technology);
